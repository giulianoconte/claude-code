...
# Claude Code sends tool args under 'tool_input'; older/test payloads use 'input'.
_ti = data.get('tool_input') or data.get('input') or {}
command = _ti.get('command', '')

if not command.strip():
    sys.exit(0)

# ── Hard-block patterns (emit 'deny' — NOT approvable in the moment) ──
# GitHub repo administration the agent must never perform on its own. If you
# genuinely need one of these, run it yourself with !<cmd>.
BLOCK_PATTERNS = [
    (r'\bgh\s+repo\s+(edit|delete|archive)', 'gh repo admin (edit/delete/archive)'),
    (r'\bgh\s+secret\b', 'gh secret'),
...
DENY_PATTERNS = [
    ...
    # Remote sync is the human's job — prompt on any push/pull. (fetch stays silent.)
    ...
    # Secret-material access via Bash (Read-tool denies don't cover shell)
    (r'\.ssh/(id_|.*_rsa|.*_ed25519)', 'ssh private key access'),
    (r'\.aws/credentials', 'aws credentials access'),
    (r'\.gnupg/', 'gnupg dir access'),
    (r'\.netrc\b', 'netrc access'),
    (r'\bid_(rsa|ed25519|ecdsa|dsa)\b', 'private key file'),
    (r'(^|\s|/)\.env(?!\.(?:example|sample|template|dist|defaults)\b)(\.|\b)', 'dotenv access'),
...
# ── Check each subcommand: hard blocks first (deny), then ask-patterns ──
